...
type validatorConfig struct {
	...
	MaxStake float32 `toml:"max_stake"`
	MaxSubValidators int `toml:"max_sub_validators"`
	MaxTimeToValidate float32 `toml:"max_time_to_validate"`
	StakeReward float32 `toml:"stake_reward"`
...
func (c Config) Parse() *tomlConfig {
	...
	if _, err := toml.DecodeFile("config.toml", &config); err != nil {
		logrus.Infoln("an error occured while loading regalio's configuration file: %v", err)
		return nil
	}
